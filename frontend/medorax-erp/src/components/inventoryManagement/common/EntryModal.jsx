...
import { postInventory, postInventoryAction } from "../../../services/inventoryApi";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (onSubmit) {
        ...
        await postInventoryAction(endpoint, values);
      } else {
      ...
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-surface-container-lowest shadow-xl">
        <div className="flex items-center justify-between border-b border-outline-variant bg-surface-container-low px-6 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-outline transition hover:bg-surface-container hover:text-on-surface-variant"
        ...
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {fields.map((field) => {
              ...
              return (
                <div
                  key={field.name}
                  className={`${field.fullWidth ? "md:col-span-2" : ""} flex flex-col gap-1.5`}
                >
                  <label
          ...
          {feedback && (
            <div
              className={`mt-4 flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-medium ${feedback.type === "success" ? "bg-secondary-container text-on-secondary-fixed-variant" : "bg-error-container text-on-error-container"}`}
              role="status"
            >
        ...
        <div className="flex justify-end gap-3 border-t border-outline-variant bg-surface-container-low px-6 py-4">
          <button
            ...
            onClick={onClose}
            disabled={submitting}
            className="h-11 rounded-xl border border-outline-variant bg-surface-container-lowest px-5 text-sm font-medium text-on-surface-variant transition hover:bg-surface-container disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            ...
            disabled={submitting}
            className="flex h-11 items-center justify-center gap-2 rounded-xl bg-primary px-6 text-sm font-semibold text-on-primary shadow-md shadow-primary/20 transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60"
          >
