...
import { downloadInventoryCsv } from "../../../services/inventoryApi";
...
const FilterBar = ({
  ...
  const updateFilter = (name, value) => {
    ...
    window.dispatchEvent(
      new CustomEvent("inventory-filters-change", { detail: next }),
    );
  };
  const handleAction = async () => {
    if (actionLabel.toLowerCase().includes("export")) {
      downloadInventoryCsv(filters);
      return;
    }
    try {
      await onAction?.();
      ...
      window.dispatchEvent(
        new CustomEvent("inventory-action-error", { detail: error }),
      );
    }
  };
  return (
      ...
      <div className="flex w-full flex-col gap-3 sm:flex-row md:w-auto">
        {categories.length > 0 && (
          <select
            ...
            className="h-11 rounded-xl border border-outline-variant bg-surface-container-lowest px-3 text-sm text-on-surface-variant outline-none transition focus:border-primary focus:ring-2 focus:ring-primary-fixed md:w-44"
          >
        ...
        {showExport && (
          <button
            type="button"
            onClick={() => downloadInventoryCsv(filters)}
