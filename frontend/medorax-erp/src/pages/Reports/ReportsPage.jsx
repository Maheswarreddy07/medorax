...
import { TABS, salesData, purchaseData, inventoryData, gstData, profitData, supplierData } from "../../data/reports/data";
...
export default function ReportsPage() {
  ...
  const [gstFromDate, setGstFromDate] = useState(getFirstDayOfMonthISO());
  const [gstToDate, setGstToDate] = useState(getTodayISO());
  ...
  // Instant CSV Export Handler
  const handleExportCSV = async () => {
    let headers = [];
    let rowsData = [];
    let filename = `${activeTab}_report_${new Date().toISOString().slice(0, 10)}.csv`;

    if (activeTab === "customer") {
      headers = ["Name", "Type", "Orders", "Spend", "Last Purchase"];
      try {
        const params = new URLSearchParams({
          customerType: customerType || "All Customers",
          ...(search && { search }),
        });
        const res = await fetch(`${API_BASE_URL}/api/customers/?${params.toString()}`);
        const json = await res.json();
        if (json && json.rows) {
          rowsData = json.rows.map((r) => [
            `"${(r.name || "").replace(/"/g, '""')}"`,
            `"${(r.type || "").replace(/"/g, '""')}"`,
            r.orders || 0,
            `"${(r.spend || "").replace(/"/g, '""')}"`,
            `"${(r.lastPurchase || "").replace(/"/g, '""')}"`,
          ]);
        }
      } catch (err) {
      ...
      headers = ["Date", "Branch", "Sales", "Transactions", "Revenue"];
      const dataSet = salesData[period]?.rows || [];
      rowsData = dataSet
        .filter((r) => branch === "All Branches" || r.branch === branch)
        .filter((r) => !search || JSON.stringify(r).toLowerCase().includes(search.toLowerCase()))
        .map((r) => [`"${r.date}"`, `"${r.branch}"`, `"${r.sales}"`, `"${r.trans}"`, `"${r.rev}"`]);
    } else if (activeTab === "purchase") {
      headers = ["Date", "Supplier", "Items", "Amount", "Status"];
      const dataSet = purchaseData.rows || [];
      rowsData = dataSet
        .filter((r) => status === "All Status" || r.status === status)
        .filter((r) => supplier === "All Suppliers" || r.supplier === supplier)
        .filter((r) => !search || JSON.stringify(r).toLowerCase().includes(search.toLowerCase()))
        .map((r) => [`"${r.date}"`, `"${r.supplier}"`, `"${r.items}"`, `"${r.amount}"`, `"${r.status}"`]);
    } else if (activeTab === "inventory") {
      if (inventorySubtype === "Stock Movement") {
        headers = ["Date", "Product", "Type", "Quantity", "Branch"];
        const dataSet = inventoryData.rows["Stock Movement"] || [];
        rowsData = dataSet
          .filter((r) => !search || JSON.stringify(r).toLowerCase().includes(search.toLowerCase()))
          .map((r) => [`"${r.date}"`, `"${r.product}"`, `"${r.type}"`, `"${r.qty}"`, `"${r.branch}"`]);
      } else {
        headers = ["Product", "Batch", "Expiry Date", "Quantity", "Status"];
        const dataSet = inventoryData.rows["Expiry Reports"] || [];
        rowsData = dataSet
          .filter((r) => !search || JSON.stringify(r).toLowerCase().includes(search.toLowerCase()))
          .map((r) => [`"${r.product}"`, `"${r.batch}"`, `"${r.expiry}"`, `"${r.qty}"`, `"${r.status}"`]);
      }
    } else if (activeTab === "gst") {
      headers = ["Invoice", "Date", "GSTIN", "Taxable Value", "Tax Amount", "Type"];
      const dataSet = gstData.rows || [];
      rowsData = dataSet
        .filter((r) => !search || JSON.stringify(r).toLowerCase().includes(search.toLowerCase()))
        .map((r) => [`"${r.invoice}"`, `"${r.date}"`, `"${r.gstin}"`, `"${r.taxable}"`, `"${r.amount}"`, `"${r.type}"`]);
    } else if (activeTab === "profit") {
      headers = ["Date", "Product", "Cost Price", "Sale Price", "Margin %"];
      const dataSet = profitData[period]?.rows || [];
      rowsData = dataSet
        .filter((r) => profitProduct === "All Products" || r.product.includes(profitProduct))
        .filter((r) => !search || JSON.stringify(r).toLowerCase().includes(search.toLowerCase()))
        .map((r) => [`"${r.date}"`, `"${r.product}"`, `"${r.cost}"`, `"${r.sale}"`, `"${r.margin}"`]);
    } else if (activeTab === "supplier") {
      headers = ["Supplier Name", "Total Supplied", "Last Order", "Outstanding"];
      const dataSet = supplierData.rows || [];
      rowsData = dataSet
        .filter((r) => !search || JSON.stringify(r).toLowerCase().includes(search.toLowerCase()))
        .map((r) => [`"${r.name}"`, `"${r.totalSupplied}"`, `"${r.lastOrder}"`, `"${r.outstanding}"`]);
    }
  ...
  const renderStats = () => {
    switch (activeTab) {
      ...
      case "customer":
        return <CustomerStats customerType={customerType} search={search} />;
      case "supplier":
